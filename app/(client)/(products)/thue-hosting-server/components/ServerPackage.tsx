  ...
  useEffect(() => {
    if (inView && swiperInstance) {
      try {
        ...
        // @ts-ignore - autoplay may be undefined depending on modules
  ...
  return (
    <div className='relative w-full'>
      <div className='custom-container-new flex flex-col items-center gap-11'>
        ...
        <div ref={swiperInViewRef} className='md:hidden w-full'>
          ...
          >
            {packages.map((pkg, index) => (
              <SwiperSlide key={`m-${pkg.id}`}>
                <div className="px-5">
                  <div className={`${pkg.containerClass} cursor-pointer`}>
                    ...
                    <div className='relative flex flex-col items-center justify-center gap-10 pb-4'>
                      <div className='pt-4 flex flex-col items-center justify-center gap-6 w-full z-[2]'>
                        ...
                        <div className='flex flex-col gap-2 px-4'>
                          ...
                          <div className='flex gap-2.5'>
                            ...
                            <span className={`text-sm 2xl:text-base font-semibold ${pkg.giftTextClass}`}>Tặng RAM khi đăng ký ít nhất 12 tháng</span>
                          </div>
                        </div>
                      </div>
                      <motion.button
                      ...
                      >
                        Đăng ký
                        <motion.div className='p-2 rounded-full bg-white' whileHover={{ x: 4 }} transition={{ type: 'spring', stiffness: 260, damping: 18 }}>
        ...
        {/* Desktop Grid */}
        <div className='hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-20 w-full'>
          {packages.map((pkg, index) => (
            <motion.div
              key={pkg.id}
              className={`${pkg.containerClass} cursor-pointer`}
