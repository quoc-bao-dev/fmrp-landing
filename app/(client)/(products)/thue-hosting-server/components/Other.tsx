...
const Other = () => {
  ...
  return (
    <div className='custom-container-new flex flex-col gap-10 xl:gap-20'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6 xl:gap-10 2xl:gap-20'>
        {otherCards.map((card, index) => (
          <Link key={index} href={card.link} target='_blank' className='relative group cursor-pointer'>
            <div className='relative w-full aspect-[416/483] flex flex-col justify-between gap-3 rounded-[32px] bg-[#EEEFFE] group-hover:bg-[#5856D6] transition-colors duration-300 border border-white'
              ...
              <h3 className='px-9 md:px-4 lg:px-9 py-4 md:py-2 lg:py-4 border-b-2 border-[#BA90FF59] text-xl md:text-base lg:text-xl 2xl:text-2xl font-bold text-[#58576B] group-hover:text-white transition-colors duration-300'>{card.title}</h3>
            ...
            <div className="absolute z-[20] bottom-0 right-0 w-[28%] rounded-full p-5 md:p-3 lg:p-5 bg-[#1D2939] group-hover:bg-[#5856D6] transition-colors duration-300">
