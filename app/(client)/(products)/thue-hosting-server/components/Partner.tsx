...
import LogoMarquee from "@/app/(client)/home/components/ui/customer-partner/LogoMarquee";
    ...
    <div className="py-0 xl:py-12 bg-white px-2 xl:px-0">
      <div className="custom-container-new flex flex-col items-center justify-center 3xl:gap-12 xl:gap-10 gap-6 relative z-[1]">
        ...
        <ButtonAnimationNew
          ...
          icon={
            <div className="text-[#8237FF] 2xl:size-12 md:size-10 size-9 rounded-full capitalize flex items-center justify-center xl:group-hover:bg-[#8237FF] group-hover:text-white duration-500 transition-colors">
              <motion.div
                initial={{ x: 0, y: 0 }}
              ...
              >
                {isHovered ? (
                  <ArrowUpRightIcon className="2xl:size-6 md:size-5 size-4 text-[#8237FF] group-hover:text-white " />
                ) : (
                  <ArrowUpRightLinearBlueIcon className="2xl:size-6 md:size-5 size-4 text-[#8237FF] " />
                )}
              </motion.div>
            </div>
          }
          ...
          reverse={true}
          className={`text-[#8237FF] border border-[#8237FF] flex items-center gap-2 3xl:!text-lg xl:!text-base lg:!text-sm md:!text-base text-sm !tracking-[1%] group hover:!backdrop-blur-[100px] hover:!backdrop-filter font-medium pl-6 pr-1 py-1 rounded-[40px] lg:w-fit w-full
            ${!isVisibleTablet ? "border-gradient-button-no-bg-fmrp" : "border-[#8237FF] text-[#8237FF]"}`}
          style={{
