...
const Why = () => {
  ...
  return (
    <div className='relative w-full'>
      <div className='custom-container-new grid grid-cols-2 lg:grid-cols-3 gap-2 xl:gap-10'>
        <div className='col-span-2 lg:col-span-1 lg:row-span-2 flex justify-center items-center'>
          <h2 className='text-center text-title-section-small text-light-1000 font-extrabold capitalize'>
            <AnimatedTitle className='text-title-section-small text-light-1000 font-extrabold' heroPerTitle={titlePart1} delay={0.1} />
            <AnimatedTitle className='text-title-section-small font-extrabold text-[#10805B]' heroPerTitle={titlePart2} delay={0.6} />
          </h2>
        </div>
        {reasons.map((reason, index) => (
          <motion.div
            key={index}
            className='p-2 lg:p-3 2xl:p-6 flex gap-2 lg:gap-4 items-center rounded-2xl xl:rounded-3xl 2xl:rounded-[32px] bg-white shadow-[0px_4px_20px_-5px_#0000001A,0px_4px_20px_-5px_#0000001A] cursor-pointer'
          ...
          >
            <Image src={reason.icon} alt='experience' width={1000} height={1000} className='w-[32px] xl:w-[80px] object-cover' />
      ...
      <Image src={IMAGES.blurBlueGreen} alt="blurBlueGreen" width={1000} height={1000} className='absolute top-[-110%] right-0 w-full xl:w-1/2 object-cover z-[-1] pointer-events-none' />
    </div>
  )
}
