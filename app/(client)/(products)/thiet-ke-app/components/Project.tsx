...
const Project = () => {
  ...
  return (
    <div className='relative flex flex-col gap-4 xl:gap-10'>
      ...
      <div className='flex overflow-x-auto xl:grid grid-cols-3 gap-y-10 gap-x-2 xl:gap-x-[100px] xl:max-w-[1016px] mx-auto'>
        {projects.map((project, index) => (
          <div key={index} className='flex flex-col items-center'>
            ...
            <div className='flex flex-col gap-2 xl:gap-4 items-center justify-center'>
              <button className='text-xs xl:text-base font-bold bg-gradient-to-r from-[#F3654A] to-[#FFB9AC] hover:from-[#F3654A]/80 hover:to-[#FFB9AC]/80 transition-all duration-300 border border-orange-300 rounded-full py-1 xl:py-2 px-4 text-white shadow-[0px_-1px_2px_0px_#FFFFFF4D_inset,0px_-2px_5px_1px_#FFFFFF1F_inset,0px_1px_2px_0px_#151A364D_inset,0px_2px_6px_0px_#151A3626_inset,0px_-2px_14px_0px_#FFFFFF26_inset,0px_20px_26px_-8px_#0F163A26]'>Xem chi tiết</button>
              <Image src={project.qr} alt='qr' width={1000} height={1000} className='size-10 xl:size-[82px] rounded-2xl object-cover' />
            </div>
          </div>
        ))}
        <div className='hidden xl:block py-3 px-6 my-auto mx-auto w-[80%] aspect-square rounded-[20px] bg-gradient-to-r from-white to-white/20 shadow-[5px_5px_20px_0px_#EB575740] relative'>
