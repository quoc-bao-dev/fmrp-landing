...
// Component for AI text with icon - can be used inline in JSX
const AILabel = ({
  text = "AI",
  isEnabled = true,
}: {
  ...
  return (
    ...
    >
      <Image
        ...
        width={10}
        height={10}
        className="flex-shrink-0 size-[16px] -mt-1"
      />
      <span className={isEnabled ? "ai-shine-text" : ""}>{text}</span>
    </span>
  );
};

// Helper function to recursively clone AILabel components with isEnabled
const cloneWithIsEnabled = (
  ...
  if (node.type === AILabel) {
    return React.cloneElement(node, { isEnabled } as any);
  }
  ...
  if (props && props.children) {
    return React.cloneElement(
      ...
      ...React.Children.toArray(props.children).map((child) =>
        cloneWithIsEnabled(child, isEnabled)
      )
    );
  }
...
// Helper component to wrap label and provide isEnabled context
const LabelWrapper = ({
  children,
  isEnabled,
  className,
  ...
  },
  {
    label: (
      <div className="flex items-center gap-1">
        <AILabel text="Tích hợp AI cơ bản" />
      </div>
    ),
    key: "ai_basic",
    freemium: true,
                ...
                {typeof f.label === "string" ? (
                  ...
                  >
                    ...
                    {f.isComingSoon && f[variant] && (
                      <span
                        className="ml-1 inline-flex items-center justify-center gap-1 rounded px-2  text-center text-[10px] md:text-[11px] font-semibold leading-[170%] text-[#F3654A] bg-[#FDE0DB]"
                      >
                        Sắp ra mắt
                      </span>
                    )}
      ...
      <div className={`${variant === "freemium" ? "mt-4 xl:mt-8 3xl:mt-10" : "3xl:mt-3 mt-2"}`}>
        {variant !== "freemium" ? (
          <ButtonAnimationNew
            ...
            onClick={() => {
              window.open(
                "https://id.zalo.me/account?continue=http%3A%2F%2Fzalo.me%2Ffososoft"
              );
            }}
            reverse={true}
