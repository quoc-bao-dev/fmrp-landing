...
const FeedbackMarquee: React.FC<FeedbackMarqueeProps> = ({ feedbacks }) => {
    ...
    return (
        ...
        <div className="relative flex justify-between w-full h-[400px] overflow-hidden">
            <motion.div
                className="w-fit flex flex-col gap-4"
                animate={{ y: ["0%", "-50%"] }}
                transition={{
                    duration: 10, // Thời gian cuộn (điều chỉnh cho phù hợp)
                    ease: "linear",
                    repeat: Infinity,
                }}
            >
                {firstColumn.map((feedback, index) => (
                    <div
                        ...
                        className="w-full bg-white p-4 shadow-lg rounded-lg overflow-hidden flex flex-col items-center justify-center h-[120px]"
                    >
                        <div className="flex items-center space-x-4">
                        ...
                        <p className="text-gray-600 text-center">"{feedback.message}"</p>
                    </div>
                ))}
            </motion.div>
            <motion.div
