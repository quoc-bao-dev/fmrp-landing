        ...
        <div className="grid grid-cols-16 items-center justify-center lg:gap-10 gap-4 py-10 w-full h-full">
            {/* Danh mục bên trái */}
            {
                isVisibleTablet
                    ...
                    >
                        <div className='flex flex-row gap-6 w-full md:max-w-[700px] max-w-[360px] overflow-x-auto'>
                            {
                                categories.map((item) => (
                                    <motion.div
                                        key={item.id}
            ...
            {/* Hình ảnh bên phải */}
            <div className="relative lg:col-span-10 col-span-16 lg:order-1 order-0 aspect-2/1 w-full h-auto">
                <AnimatePresence mode="wait">
                    <motion.div
