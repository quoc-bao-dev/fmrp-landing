...
const values = [
  ...
  },
  {
    ...
    description:
      "Vận dụng công nghệ vào trong việc quản lý thay vì thủ công, excel, zalo...",
  },
  {
  ...
  },
  {
    ...
    description:
      "Sự tăng trưởng sản lượng, chi phí các nguồn lực trong xưởng và xí nghiệp",
  },
  {
          ...
          <h2 className="space-x-2 font-extrabold lg:text-start text-center">
            <span
              ...
              style={{
                ...
                background:
                  "linear-gradient(-90deg, #0375F3 0%, #036EEA 11%, #0267E1 22%, #0261D7 33%, #025ACE 44%, #0254C5 56%, #024EBC 67%, #0148B3 78%, #0142A9 89%, #013DA0 100%)",
            ...
            <span className="text-title-section-small text-[#1A2025] capitalize">
              Có Thể Giúp Gì Cho Các Nhà Sản Xuất?
            </span>
          </h2>
        </div>
          ...
          <div className="w-full grid grid-cols-2 md:grid-cols-2 3xl:gap-x-8 xl:gap-x-6 md:gap-x-4 md:gap-y-0 gap-4 auto-rows-auto relative z-[10]">
            {values.map((value, index) => (
              <AnimatedReveal
                ...
                className={`relative ${
                  index % 2 !== 1
                    ? "3xl:mt-8 xl:mt-6 md:mt-4"
                    : "3xl:mb-8 xl:mb-6 md:mb-4" // Đẩy cột 2 xuống
                }`}
              >
                <WhyCard
                  className="lg:p-6 !p-4"
                  linearImageColor={
                    "linear-gradient(270deg, rgba(3, 117, 243, 0) 0%, #0375F3 32%, #0375F3 61.5%, rgba(3, 117, 243, 0) 100%)"
                  }
                  {...value}
                />
              </AnimatedReveal>
